...
const supported = ["login", "signup", "verify-email", "forgot-password", "reset-password", "pricing", "dashboard", "project", "cowork", "settings", "admin", "terms", "privacy"];

export default async function CatchAllPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
  if (!supported.includes(slug[0])) notFound();
  if (slug[0] === "login" || slug[0] === "signup") return <AuthPanel mode={slug[0]} />;
  ...
  if (["dashboard", "project", "cowork", "settings", "admin"].includes(slug[0])) {
    const db = await createServerSupabaseClient();
    if (!db) redirect("/login");
    const { data: { user } } = await db.auth.getUser();
    if (!user || !user.email_confirmed_at) redirect("/login");
    if (slug[0] === "admin") {
      const { data, error } = await db.from("platform_owners").select("user_id").eq("user_id", user.id).maybeSingle();
