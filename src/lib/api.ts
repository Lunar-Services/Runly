...
export async function session() {
  ...
  if (!db) {
    console.error(
      "Runly setup: Supabase URL and publishable key are missing. See SETUP.md.",
    );
    throw new ApiError(
      503,
...
export async function adminSession() {
  ...
  const { data, error } = await value.db
    .from("account_roles")
    .select("role")
    ...
    .maybeSingle();
  if (error || !data)
    throw new ApiError(403, "This page is only available to an administrator.");
  return value;
}
export function sameOrigin(request: Request) {
  const expected = process.env.RUNLY_SITE_URL
...
export function appOrigin(request: Request) {
  if (process.env.RUNLY_SITE_URL)
    return new URL(process.env.RUNLY_SITE_URL).origin;
...
export async function rateLimit(
  ...
  if (
    ...
    !new URL(process.env.RUNLY_SITE_URL).hostname.match(
      /^(localhost|127\.0\.0\.1)$/,
    )
  ) {
  ...
  if (localBuckets.size > 5000)
    for (const [id, entry] of localBuckets)
      if (entry.until < now) localBuckets.delete(id);
  const bucket = localBuckets.get(key);
  if (!bucket || bucket.until < now)
    localBuckets.set(key, { count: 1, until: now + 60_000 });
